...
export class gActorSheet extends ActorSheet {
   ...
   activateListeners(html) {
      ...
      html.find('.badge-click').click(async (ev) => {
         ...
         await this.actor.update({
            [`data.attributes.${attKey}.value`]: stringvalue
         });
      ...
      html.find('.roll-mode').click(ev => {
         ...
         this.actor.update({
            "data.rollmode": actorData.rollmode
         }, {
            diff: false
         });
   ...
   async exportHTML(htmlObject, filename) {
      ...
      const blob = new Blob([htmlObject], {
         type: 'text/html'
      });
   ...
   async checkConsistency() {
      ...
      for (let i = 0; i < gamecItems.length; i++) {
         ...
         await mycitem.update({
            "data": mycitem.data.data
         });
      }
   ...
   async buildHTML(tabs) {
      ...
      if (game.settings.get("starwars-d6", "consistencycheck") != "") {
            ...
            one: {
               ...
               callback: async (html) => {
                  if (!disabled) {
                     ...
                     await this.actor.update({
                        "data.citems": this.actor.data.data.citems
                     }, {
                        diff: false
                     });
                  }
   ...
   async clickRadioInput(clickValue, propId, target) {
      ...
      await this.actor.update({
         [`data.attributes.${attKey}.value`]: clickValue
      });
      //await this.actor.actorUpdater();
